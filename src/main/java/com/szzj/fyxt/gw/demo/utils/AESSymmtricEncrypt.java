...
public class AESSymmtricEncrypt {
     ...
     * 支持的加密padding
     */
    private static final Set<String> SUPPORTED_ENCRYPT_PADDING = new HashSet<>(Arrays.asList("PKCS5Padding",
        "NoPadding"));

    public static String encrypt(String content, String token) {
        try {
            return encrypt(content, token, "AES/ECB/PKCS5Padding", "utf-8", null);
        } catch (Exception e) {
    ...
    public static String decrypt(String content, String token) {
        try {
            return decrypt(content, token, "AES/ECB/PKCS5Padding", "utf-8", null);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    public static String encrypt(String content, String key, String algorithm, String charset,
        Map<String, Object> operationContext) throws UnsupportedEncodingException,
        GeneralSecurityException {
        byte[] keyData = Base64.decodeBase64(key); //密钥数据
        List<String> r = Arrays.asList(StringUtils.split(algorithm, '/'));
        byte[] cipherBytes = symmtricCrypto(content.getBytes(charset), keyData, r.get(0), r.get(1),
            r.get(2), Cipher.ENCRYPT_MODE);
        return Base64.encodeBase64String(cipherBytes);
    }

    public static String decrypt(String content, String key, String algorithm, String charset,
        Map<String, Object> operationContext) throws UnsupportedEncodingException,
        GeneralSecurityException {
        byte[] bytes = Base64.decodeBase64(content); //待加/解密的数据
        byte[] keyData = Base64.decodeBase64(key); //密钥数据
        List<String> r = Arrays.asList(StringUtils.split(algorithm, '/'));
